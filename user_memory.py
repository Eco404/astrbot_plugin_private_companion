...
""".strip()
    ...
    def _response_contains_explicit_adult_content(value: Any) -> bool:
        ...
        return bool(
            re.search(
                r"nsfw|色情|露骨|性行为|性交|做爱|口交|肛交|阴茎|阴道|射精|高潮|裸体|全裸|性器官|乳房",
                text,
                re.IGNORECASE,
            )
        )
        ...
        *,
        ...
        if isinstance(music_album_context, dict) and self._music_album_reply_needs_disambiguation_fix(response_text):
            ...
            if fallback:
                ...
                return fallback
        content_tier = self._response_content_tier(review_event)
        if content_tier != "adult" and self._response_contains_explicit_adult_content(response_text):
            logger.warning("[PrivateCompanion] 回复超出统一内容尺度，已本地收敛: tier=%s", content_tier)
            return self._content_tier_boundary_reply()
...
【统一内容尺度】
{content_tier}；normal 不升级暧昧或成人内容，flirt 只允许非露骨暧昧，adult 也只能承接本轮明确同意的合法成年人私聊。
