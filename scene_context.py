...
class SceneContextMixin:
    ...
    def _build_companion_scene_snapshot(
        ...
        *,
        ...
        afterglow_getter = getattr(self, "_game_afterglow_public_view", None)
        game_afterglow = (
            afterglow_getter(current_user.get("game_afterglow"))
            if callable(afterglow_getter)
            else {}
        )
        ...
        if purpose not in {"selfie_scene", "image_search"}:
            ...
            if bool(game_afterglow.get("active")):
                ...
                parts.append(
                    ...
                    "只作为语气和是否想再玩的底色，不要复述内部状态或把正常胜负说成关系受伤"
                )
