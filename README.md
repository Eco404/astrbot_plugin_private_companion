...
- AstrBot 版本：`>=4.22.0`
- 编码要求：UTF-8
- 维护状态：5.10.0 将同源情境表达与语法习惯合并为规则组，支持整组审核、按组召回和旧规则自动分组，并包含 MemoryCompanion 历史关系网协同与 Proactive Chat 私聊主动发送联动。
...
`陪伴 答疑`现在按“**现象 → 当前证据 → 一步检查**”工作，不会默认把功能说明和一长串参数念出来。
...
- 普通排障默认只给一个最可能原因、能对上的运行态证据和一个下一步动作；证据不足时会明确说明只是倾向判断。
...
- 相关设置：主动图片分享、生图后端、在线生图模型平台、文生图工作流、自拍工作流、参考图一致性（可选）、人设参考图路径（可选）、群聊成图安全审核与私聊回退（可选）、每日穿搭照片、自然语言生图/改图、自然语言生图附加提示词、生图场景预设、本地图片读取保护。
...
- 开启后，自动角色出镜链路会优先使用今日穿搭图，再回退到配置的人设参考图路径或 URL；路径会保留，开关关闭不会删除。
