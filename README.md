...
- 在线生图模型平台。
- 主/备在线 API 可在拓展页一键交换，也可用聊天指令 `陪伴 查看生图API` 查看摘要、`陪伴 切换生图API` 交换主用与备选在线 API；该指令需要插件管理权限，不会回显 API Key。
- 外部图片 API 地址。
- 外部图片 API 密钥。
- 外部图片模型名。
- 外部图片尺寸。
...
主要用户和次要用户会使用不同边界。主要用户可以继承基础人格中的专属亲近关系、长期陪伴动机和更丰富的主动能力；次要用户只保留普通朋友式的轻量关心、共同话题和必要转告。即使基础人格里写了“主人”“恋人”或专属称呼，次要用户私聊也会先做身份防串，再按当前 QQ 精确注入低风险关系网资料。
