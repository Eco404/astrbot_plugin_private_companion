...
- AstrBot 版本：`>=4.22.0`
- 编码要求：UTF-8
- 维护状态：5.9.4 修复自定义函数工具生图调用链，并新增动作查岗预约和每张模型卡片独立超时配置。
...
可选的“余额感知与零花钱主动”也接在这一层。开启后，插件按配置周期读取一个 HTTP JSON 余额接口，支持自定义鉴权、JSON 路径、单位换算，以及绝对余额和剩余比例两套阈值。余额进入偏低或危急档时，它只给主要用户放入一个候选念头，不会直接发送固定话术；后续仍要经过人格、关系、免打扰、未回应降速、每日额度和发送终审。持续低余额默认 24 小时最多重新尝试一次，危急档使用一半冷却，恢复正常后自动解除低余额状态。

生成层会把这项状态解释为 Bot 自身服务或能力的可支配经费，可按人格和世界观自然说成零花钱、饭卡、钱包、电量或补给。它不会把余额写成用户欠款，也会明确禁止催转账、道德绑架、用断联或停止陪伴施压，以及泄露接口地址、Key、JSON 字段、模型或 Token 等内部信息。该能力默认关闭；建议先确认接口取值与单位，再启用主动开关。
