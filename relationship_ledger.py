...
def apply_relationship_event(
    ...
    *,
    ...
    dedupe_key = _event_key(reason, ts, event_id, max(60, int(event_window_seconds or DEFAULT_EVENT_WINDOW_SECONDS)))
    if any(isinstance(item, dict) and item.get("event_key") == dedupe_key for item in ledger[-200:]):
...
def apply_natural_relationship_decay(
    ...
    *,
    ...
    last_day = datetime.fromtimestamp(last_effective, tz=timezone.utc).date()
...
def _event_key(reason: str, ts: float, event_id: Any, window_seconds: int) -> str:
    ...
    source = (
        ...
        if explicit
        else f"{canonical_reason}:{int(ts // window_seconds)}"
    )
