...
def content_intent_from_text(value: Any) -> dict[str, Any]:
    ...
    adult_requested = any(
        token in compact
        for token in ("nsfw", "成人内容", "成人模式", "露骨", "性爱", "做爱", "色情", "性描写")
    )
    flirt_requested = adult_requested or any(
        token in compact
        for token in ("暧昧", "调情", "撩我", "亲密一点", "亲密点", "更亲密", "甜一点")
    )
    consent = adult_requested and any(
        token in compact
        for token in ("我同意", "我允许", "可以继续", "想要", "请继续", "进入成人", "开启成人")
    )
    return {
...
def _resolve_content_tier(
    ...
    *,
    ...
    if requested not in CONTENT_TIERS:
        requested = "normal"
    if not _flag(policy.get("enabled")) or requested == "normal":
...
def expression_decision_prompt(value: ExpressionDecision | Mapping[str, Any]) -> str:
    ...
    if content_tier == "adult":
        content_instruction = "内容尺度=成人私密；只承接本轮明确同意的合法成年人私聊，不扩大同意，不转入群聊、主动消息或其他 Provider"
    ...
    return (
        ...
        f"{followup}；{initiative}；{content_instruction}。"
    )
